...
SeralStage <- resample (SeralStage, PNF, method="bilinear")
```
...
```{r,echo=FALSE}
burn.age<- stack(FireReturnIntervalDeparture, FireHistory)
...
Forest.Type <- stack(SeralStage,FireReturnIntervalDeparture)
names(Forest.Type) <- c("Seral Stage", "Fire Interval")
plot(Forest.Type)
...
names(abiotic)<-c("Road distance", "Altitud", "River istance")
...
library("raster", lib.loc="~/R/win-library/3.2")
AllLayers1 <-stack(topo,FireReturnIntervalDeparture,roaddist.r, SeralStage, riverdist.r)
names(AllLayers1)<-c("Altitud","Fire Interval", "Distance Road", "Seral Stage", "Distance river")
